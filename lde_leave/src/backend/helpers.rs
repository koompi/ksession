...
pub(super) fn dbus_call(service: &str, path: &str, interface: &str, conn: &Connection, method: &str, dur: Duration) -> Result<bool, Error> {
    let proxy = conn.with_proxy(service, path, dur);
    let (result,) = proxy.method_call(interface, method, ())?;
    Ok(result)
}

pub(super) fn dbus_call_init_system(service: &str, path: &str, interface: &str, conn: &Connection, method: &str, need_arg: Option<bool>, dur: Duration) -> Result<bool, Error> {
    let proxy = conn.with_proxy(service, path, dur);
...
pub(super) fn dbus_call_init_system_with_args(service: &str, path: &str, interface: &str, conn: &Connection, method: &str, args: Option<&str>, dur: Duration) -> Result<bool, Error> {
    let proxy = conn.with_proxy(service, path, dur);
