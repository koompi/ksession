...
impl AutostartPage {
   ...
   pub fn save(&mut self) {
      ...
      let prev_items: HashMap<String, AutostartItem> = AutostartItem::create_item_map().into_iter().filter(|(_, item)| !AutostartUtils::is_lde_module(item.file())).collect();
      let current_items: HashMap<String, AutostartItem> = self.items().into_iter().filter(|(_, item)| !AutostartUtils::is_lde_module(item.file())).collect();
