...
impl AutoStartEditPage {   
   ...
   pub fn update(&mut self, msg: AutoStartEditPageMsg) {
      ...
      match msg { 
         ...
         BtnSearchClicked(is_searching) => self.is_searching = is_searching,
         BtnCancelClicked => {},
   ...
   pub fn view(&mut self) -> Element<AutoStartEditPageMsg> {
      ...
      let btn_search = Button::new(btn_search_state, Text::new("  Search  ")).on_press(AutoStartEditPageMsg::BtnSearchClicked(!(*is_searching))).style(CustomButton::Default);
