...
impl AutostartPage {
    ...
    fn save(&mut self) {
        ...
        let prev_items: HashMap<String, AutostartItem> = AutostartItem::create_item_map().into_iter().filter(|(key, item)| AutostartUtils::is_lde_module(item.file())).collect();
        if let Some(current_model) = self.model {
            let current_items: HashMap<String, AutostartItem> = current_model.items().into_iter().filter(|(key, item)| AutostartUtils::is_lde_module(item.file())).collect();
