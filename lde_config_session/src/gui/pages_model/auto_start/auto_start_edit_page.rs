use crate::gui::{CustomButton, CustomCheckbox, CustomTextInput};
use iced::{
    button, text_input, Align, Button, Checkbox, Column, Container, Element, Length, Row, Space,
    Text, TextInput,
};
use nfd2::Response;
...
impl AutostartEditPage {
    ...
    pub fn update(&mut self, msg: AutostartEditMsg) {
        use AutostartEditMsg::*;

        match msg {
            TxtNameChanged(val) => self.name_val = val,
            TxtCmdChanged(val) => self.cmd_val = val,
            BtnSearchClicked => {
                match nfd2::open_file_dialog(None, Some(std::path::Path::new("/usr/bin")))
                    .expect("oh no")
                {
                    Response::Okay(file_path) => {
                        self.cmd_val = file_path.into_os_string().into_string().unwrap()
                    }
                    _ => {}
                }
            }
    ...
    pub fn view(&mut self) -> Element<AutostartEditMsg> {
        ...
        let txt_name = TextInput::new(
            txt_name_state,
            "",
            name_val,
            AutostartEditMsg::TxtNameChanged,
        )
        .padding(10)
        .width(Length::Fill)
        .style(CustomTextInput::Default);
        let txt_cmd = TextInput::new(
            txt_cmd_state,
            "",
            cmd_val,
            AutostartEditMsg::TxtCmdChanged,
        )
        .padding(10)
        .width(Length::Fill)
        .style(CustomTextInput::Default);
        let btn_search = Button::new(btn_search_state, Text::new("  Search  "))
            .on_press(AutostartEditMsg::BtnSearchClicked)
            .style(CustomButton::Default);
        let chb_wait_for = Checkbox::new(
            *wait_for_system_tray,
            "Wait for system tray",
            AutostartEditMsg::WaitForSystemTrayToggled,
        )
        .spacing(10)
        ...
        if !name_val.is_empty() && !cmd_val.is_empty() {
            btn_ok = btn_ok.on_press(AutostartEditMsg::BtnOkClicked(
                *wait_for_system_tray,
                name_val.clone(),
                cmd_val.clone(),
            ));
        ...
        let btn_cancel = Button::new(btn_cancel_state, Text::new("  Cancel  "))
            .on_press(AutostartEditMsg::BtnCancelClicked)
            .style(CustomButton::Default);
